...
console.log(`${colors.bold(colors.green('Building'))} deno_doc web assembly...`);

const home = Deno.env.get('HOME');
const root = path.dirname(path.fromFileUrl(import.meta.url));
const copyrightHeader = `// Copyright 2020-${new Date().getFullYear()} the Deno authors. All rights reserved. MIT license.`;
...
console.log(`${colors.bold(colors.green('Copying'))} lib wasm...`);

const denoDocWasmDest = './lib/deno_doc_bg.wasm';
await Deno.copyFile('./target/wasm32-bindgen-deno-js/deno_doc_bg.wasm', denoDocWasmDest);
...
const generatedJs = await Deno.readTextFile('./target/wasm32-bindgen-deno-js/deno_doc.js');
...
const denoFmtCmd = ['deno', 'fmt', '--quiet', './lib/deno_doc.generated.js'];
...
console.log(`${colors.bold(colors.green('Finished'))} deno_doc web assembly.`);
