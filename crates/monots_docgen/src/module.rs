use deno_ast::{ParsedSource, SourceRanged, SourceRangedForSpanned};
...
pub fn get_doc_node_for_export_decl(
  ...
  match &export_decl.decl {
    ...
    Decl::Fn(fn_decl) => {
      let (name, fn_def) = super::function::get_doc_for_fn_decl(parsed_source, fn_decl);
    ...
    Decl::Var(var_decl) => {
      ...
      DocNode::variable(name, location, DeclarationKind::Export, js_doc, var_def)
    }
    ...
    Decl::TsModule(ts_module) => {
      let (name, namespace_def) =
        super::namespace::get_doc_for_ts_module(doc_parser, parsed_source, ts_module);
      DocNode::namespace(
        name,
        location,
        DeclarationKind::Export,
