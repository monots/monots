...
use deno_ast::swc::ast::{TsTypeParam, TsTypeParamDecl};
...
impl From<&TsTypeParam> for TsTypeParamDef {
  fn from(param: &TsTypeParam) -> TsTypeParamDef {
    ...
    let constraint: Option<TsTypeDef> = if let Some(ts_type) = param.constraint.as_ref() {
      let type_def: TsTypeDef = (&**ts_type).into();
      Some(type_def)
    } else {
      None
    };
    let default: Option<TsTypeDef> = if let Some(ts_type) = param.default.as_ref() {
      let type_def: TsTypeDef = (&**ts_type).into();
      Some(type_def)
    } else {
      None
