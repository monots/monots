...
lazy_static! {
  static ref JS_DOC_TAG_MAYBE_DOC_RE: Regex =
    Regex::new(r#"(?s)^\s*@(deprecated|example)(?:\s+(.+))?"#).unwrap();
  static ref JS_DOC_TAG_NAMED_RE: Regex =
    Regex::new(r#"(?s)^\s*@(callback|template)\s+([a-zA-Z_$]\S*)(?:\s+(.+))?"#).unwrap();
  static ref JS_DOC_TAG_NAMED_TYPED_RE: Regex =
    Regex::new(r#"(?s)^\s*@(prop(?:erty)?|typedef)\s+\{([^}]+)\}\s+([a-zA-Z_$]\S*)(?:\s+(.+))?"#)
      .unwrap();
  static ref JS_DOC_TAG_ONLY_RE: Regex =
    Regex::new(r#"^\s*@(constructor|class|module|public|private|protected|readonly)"#).unwrap();
  ...
  static ref JS_DOC_TAG_RETURN_RE: Regex =
    Regex::new(r#"(?s)^\s*@returns?(?:\s+\{([^}]+)\})?(?:\s+(.+))?"#).unwrap();
  static ref JS_DOC_TAG_TYPED_RE: Regex =
    Regex::new(r#"(?s)^\s*@(enum|extends|augments|this|type)\s+\{([^}]+)\}(?:\s+(.+))?"#).unwrap();
}
...
mod tests {
  ...
  fn test_js_doc_tag_only() {
    assert_eq!(
      serde_json::to_value(JsDoc::from("@constructor more".to_string())).unwrap(),
...
"#
  ...
  fn test_js_doc_tag_maybe_doc() {
    ...
    assert_eq!(
      serde_json::to_value(JsDoc::from("@example\n\nconst a = \"a\";\n".to_string())).unwrap(),
      json!({
        "tags": [{
          "kind": "example",
          "doc": "const a = \"a\";"
  ...
  fn test_js_doc_tag_param() {
    ...
    assert_eq!(
      serde_json::to_value(JsDoc::from("@param {string} a".to_string())).unwrap(),
      json!({
        "tags": [{
          "kind": "param",
          "name": "a",
