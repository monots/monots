...
use crate::{
  ...
  params::ts_fn_param_to_param_def,
  swc_util::{get_location, is_false, js_doc_for_range},
  ...
  match expr {
    ...
    Member(member_expr) => {
      ...
      let right = match &member_expr.prop {
        ...
        MemberProp::Computed(_) | MemberProp::PrivateName(_) => "[UNSUPPORTED]".to_string(),
      };
      format!("[{}{}]", left, right)
    }
    Lit(lit) => {
      use deno_ast::swc::ast::{BigInt, Bool, JSXText, Lit, Number, Regex, Str};
      match lit {
...
pub fn get_doc_for_ts_interface_decl(
  ...
  for type_element in &interface_decl.body.body {
    ...
    match &type_element {
      TsMethodSignature(ts_method_sig) => {
        ...
        let maybe_return_type = ts_method_sig.type_ann.as_ref().map(ts_type_ann_to_def);
      ...
      TsGetterSignature(ts_getter_sig) => {
        ...
        let maybe_return_type = ts_getter_sig.type_ann.as_ref().map(ts_type_ann_to_def);
      ...
      TsSetterSignature(ts_setter_sig) => {
        ...
        let param_def = ts_fn_param_to_param_def(Some(parsed_source), &ts_setter_sig.param);
      ...
      TsConstructSignatureDecl(ts_construct_sig) => {
        let construct_js_doc = js_doc_for_range(parsed_source, &ts_construct_sig.range());
  ...
  let type_params = maybe_type_param_decl_to_type_param_defs(interface_decl.type_params.as_ref());
