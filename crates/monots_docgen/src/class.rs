...
use crate::{
  ...
  function::{function_to_function_def, FunctionDef},
  js_doc::JsDoc,
  ...
  params::{
    ...
    param_to_param_def,
    prop_name_to_string,
    ts_fn_param_to_param_def,
  },
  swc_util::{get_location, js_doc_for_range},
  ts_type::{
    infer_ts_type_from_expr,
    maybe_type_param_instantiation_to_type_defs,
    ts_type_ann_to_def,
    TsTypeDef,
  },
...
pub fn class_to_class_def(
  ...
  for member in &class.body {
    ...
    match member {
      Constructor(ctor) => {
        ...
        let constructor_name = prop_name_to_string(Some(parsed_source), &ctor.key);
          ...
          let param_def = match param {
            ...
            TsParamProp(ts_param_prop) => {
              ...
              match &ts_param_prop.param {
                TsParamPropParam::Ident(ident) => ident_to_param_def(Some(parsed_source), ident),
      ...
      Method(class_method) => {
        let method_js_doc = js_doc_for_range(parsed_source, &class_method.range());
        let method_name = prop_name_to_string(Some(parsed_source), &class_method.key);
        let fn_def = function_to_function_def(parsed_source, &class_method.function);
      ...
      ClassProp(class_prop) => {
        ...
        let decorators = decorators_to_defs(parsed_source, &class_prop.decorators);
  ...
  let type_params = maybe_type_param_decl_to_type_param_defs(class.type_params.as_ref());
...
pub fn get_doc_for_class_decl(
  ...
  let (class_def, js_doc) = class_to_class_def(parsed_source, &class_decl.class);
