...
use deno_ast::{swc::ast::*, SourceRange, SourceRangedForSpanned};
use serde::{Deserialize, Serialize};

use crate::{
  colors,
  display::{display_computed, display_optional, display_readonly, SliceDisplayer},
  interface::expr_to_name,
  params::{pat_to_param_def, ts_fn_param_to_param_def},
  swc_util::is_false,
...
pub fn ts_entity_name_to_name(entity_name: &deno_ast::swc::ast::TsEntityName) -> String {
    ...
    for type_element in &other.members {
      ...
      match &type_element {
        TsMethodSignature(ts_method_sig) => {
          ...
          let type_params =
            maybe_type_param_decl_to_type_param_defs(ts_method_sig.type_params.as_ref());
        ...
        TsSetterSignature(ts_setter_sig) => {
          ...
          let params = vec![ts_fn_param_to_param_def(None, &ts_setter_sig.param)];
        ...
        TsPropertySignature(ts_prop_sig) => {
          ...
          let type_params =
            maybe_type_param_decl_to_type_param_defs(ts_prop_sig.type_params.as_ref());
        ...
        TsCallSignatureDecl(ts_call_sig) => {
          ...
          let type_params =
            maybe_type_param_decl_to_type_param_defs(ts_call_sig.type_params.as_ref());
        ...
        TsConstructSignatureDecl(ts_construct_sig) => {
          ...
          let type_params =
            maybe_type_param_decl_to_type_param_defs(ts_construct_sig.type_params.as_ref());
  ...
  fn from(other: &TsFnOrConstructorType) -> TsTypeDef {
    ...
    let fn_def = match other {
      TsFnType(ts_fn_type) => {
        ...
        let type_params = maybe_type_param_decl_to_type_param_defs(ts_fn_type.type_params.as_ref());
      ...
      TsConstructorType(ctor_type) => {
        ...
        let type_params = maybe_type_param_decl_to_type_param_defs(ctor_type.type_params.as_ref());
        TsFnOrConstructorDef {
          constructor: true,
...
impl From<&deno_ast::swc::ast::ArrowExpr> for TsFnOrConstructorDef {
  fn from(expr: &deno_ast::swc::ast::ArrowExpr) -> Self {
    ...
    let type_params = maybe_type_param_decl_to_type_param_defs(expr.type_params.as_ref());
...
impl From<&deno_ast::swc::ast::FnExpr> for TsFnOrConstructorDef {
  fn from(expr: &deno_ast::swc::ast::FnExpr) -> Self {
    ...
    let ts_type = expr
      ...
      .map(ts_type_ann_to_def)
      .unwrap_or_else(|| TsTypeDef::keyword("unknown"));
    let type_params = maybe_type_param_decl_to_type_param_defs(expr.function.type_params.as_ref());
...
pub fn infer_ts_type_from_expr(expr: &Expr, is_const: bool) -> Option<TsTypeDef> {
  match expr {
    Expr::Array(arr_lit) => {
      // e.g.) const n = ["a", 1];
...
fn infer_ts_type_from_arr_lit(arr_lit: &ArrayLit, is_const: bool) -> Option<TsTypeDef> {
...
fn infer_ts_type_from_const_assertion(assertion: &TsConstAssertion) -> Option<TsTypeDef> {
  ...
  fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
    ...
    match kind {
      ...
      TsTypeDefKind::Mapped => {
        ...
        let type_param = if let Some(ts_type_def) = &mapped_type.type_param.constraint {
