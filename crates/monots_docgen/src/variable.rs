...
use crate::ts_type::{infer_simple_ts_type_from_var_decl, ts_type_ann_to_def, TsTypeDef};
...
pub fn get_doc_for_var_decl(var_decl: &deno_ast::swc::ast::VarDecl) -> (String, VariableDef) {
  ...
  let maybe_ts_type = match &var_declarator.name {
    deno_ast::swc::ast::Pat::Ident(ident) => ident.type_ann.as_ref().map(ts_type_ann_to_def),
    _ => None,
  };
