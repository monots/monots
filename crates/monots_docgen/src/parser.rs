...
use deno_ast::{
  ...
  ParsedSource,
  SourceRangedForSpanned,
};
use deno_graph::{MediaType, ModuleGraph, ModuleSpecifier, Resolved, SourceParser};

use crate::{
  js_doc::JsDoc,
  namespace::NamespaceDef,
  node,
  node::{DeclarationKind, DocNode, ModuleDoc},
  swc_util::{get_location, js_doc_for_range, module_export_name_value, module_js_doc_for_source},
  ImportDef,
  Location,
  ReexportKind,
};
...
impl<'a> DocParser<'a> {
  pub fn new(graph: ModuleGraph, private: bool, ast_parser: &'a dyn SourceParser) -> Self {
    DocParser {
      ast_parser,
      graph,
      private,
  ...
  /// as well as a list of reexported items which need to be fetched from other
  /// modules.
  pub fn parse_module(
    &self,
    ...
    let mut definitions = self.get_doc_nodes_for_module_body(&parsed_source, module.body.clone());
    let import_doc_entries =
      self.get_doc_nodes_for_module_imports(&parsed_source, module.body.clone(), specifier)?;
  ...
  pub fn parse(&self, specifier: &ModuleSpecifier) -> Result<Vec<DocNode>, DocError> {
    let module = self
      ...
      .ok_or_else(|| DocError::Resolve(format!("Unable to load specifier: \"{}\"", specifier)))?;

    if let Some(source_code) = &module.maybe_source {
      self.parse_source(&module.specifier, module.media_type, source_code.clone())
    } else {
  ...
  fn flatten_reexports(
    ...
    for specifier in by_src.keys() {
      ...
      for reexport in reexports_for_specifier {
        match &reexport.kind {
          node::ReexportKind::All => processed_reexports.extend(doc_nodes.clone()),
  ...
  pub fn parse_with_reexports(
    ...
    let module = if let Some((_, Resolved::Ok { specifier, .. })) = &module.maybe_types_dependency {
      self
        .graph
        .try_get(specifier)
        .map_err(|err| DocError::Resolve(err.to_string()))?
    ...
    if let Some(source_code) = &module.maybe_source {
      let module_doc =
        self.parse_module(&module.specifier, module.media_type, source_code.clone())?;
  ...
  fn get_doc_nodes_for_module_imports(
    ...
    for node in module_body.iter() {
      if let deno_ast::swc::ast::ModuleItem::ModuleDecl(ModuleDecl::Import(import_decl)) = node {
        ...
        for specifier in &import_decl.specifiers {
          ...
          let doc_node = DocNode::import(name, location.clone(), js_doc.clone(), import_def);
      ...
      ModuleDecl::ExportDefaultDecl(export_default_decl) => {
        ...
        let doc_node = match &export_default_decl.decl {
          ...
          DefaultDecl::Fn(fn_expr) => {
            let function_def =
              crate::function::function_to_function_def(parsed_source, &fn_expr.function);
            DocNode::function(
              name,
              location,
              DeclarationKind::Export,
          ...
          DefaultDecl::TsInterfaceDecl(interface_decl) => {
            let (_, interface_def) =
              crate::interface::get_doc_for_ts_interface_decl(parsed_source, interface_decl);
            DocNode::interface(
              name,
              location,
              DeclarationKind::Export,
  ...
  pub fn get_doc_node_for_decl(
    ...
    match decl {
      Decl::Class(class_decl) => {
        ...
        let (name, class_def, _) = super::class::get_doc_for_class_decl(parsed_source, class_decl);
      ...
      Decl::Fn(fn_decl) => {
        let (name, function_def) = super::function::get_doc_for_fn_decl(parsed_source, fn_decl);
      ...
      Decl::TsInterface(ts_interface_decl) => {
        let (name, interface_def) =
          super::interface::get_doc_for_ts_interface_decl(parsed_source, ts_interface_decl);
      ...
      Decl::TsTypeAlias(ts_type_alias) => {
        let (name, type_alias_def) =
          super::type_alias::get_doc_for_ts_type_alias_decl(parsed_source, ts_type_alias);
      ...
      Decl::TsEnum(ts_enum) => {
        let (name, enum_def) = super::r#enum::get_doc_for_ts_enum_decl(parsed_source, ts_enum);
        ...
        for specifier in &import_decl.specifiers {
          let import = match specifier {
            ...
            ImportSpecifier::Namespace(namespace_specifier) => {
              Import {
                kind: ImportKind::Namespace(namespace_specifier.local.sym.to_string()),
                src: import_decl.src.value.to_string(),
              }
            }
          };
  ...
  pub fn get_reexports_for_module_body(
    ...
    if self.private {
      reexports.extend(imports.values().cloned().map(|import| {
        node::Reexport {
          src: import.src,
          kind: match import.kind {
            ImportKind::Named(orig, exported) => ReexportKind::Named(orig, exported),
                ...
                .map(|export_specifier| {
                  match export_specifier {
                    ExportSpecifier::Namespace(ns_export) => {
                      node::Reexport {
                        kind: node::ReexportKind::Namespace(module_export_name_value(
                          &ns_export.name,
                        )),
                    ...
                    ExportSpecifier::Named(named_export) => {
                      ...
                      let maybe_alias =
                        named_export.exported.as_ref().map(module_export_name_value);
              ...
              named_export
                ...
                .filter_map(|specifier| {
                  if let ExportSpecifier::Named(specifier) = specifier {
                    if let Some(import) = imports.get(&module_export_name_value(&specifier.orig)) {
                      // If it has the same name as the original import and private values are
                      // exported, don't export this again and document the
                      // same value twice.
                      ...
                      Some(node::Reexport {
                        ...
                        kind: match &import.kind {
                          ...
                          ImportKind::Namespace(_) => ReexportKind::Namespace(name),
                        },
                      })
                    } else {
                      None
  ...
  fn get_symbols_for_module_body(
    ...
    for node in module_body.iter() {
      let doc_node = match node {
        ModuleItem::Stmt(Stmt::Decl(decl)) => self.get_doc_node_for_decl(parsed_source, decl),
  ...
  pub fn get_doc_nodes_for_module_body(
    ...
    if let Some((js_doc, range)) = module_js_doc_for_source(parsed_source) {
      let doc_node = DocNode::module_doc(get_location(parsed_source, range.start), js_doc);
      ...
      match node {
        ModuleItem::Stmt(stmt) => {
          if let Stmt::Decl(decl) = stmt {
            if let Some(mut doc_node) = self.get_doc_node_for_decl(parsed_source, decl) {
        ...
        ModuleItem::ModuleDecl(module_decl) => {
          ...
          doc_entries.extend(self.get_doc_nodes_for_module_exports(parsed_source, module_decl));
