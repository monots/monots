...
use crate::{
  colors,
  display::{display_abstract, display_async, display_generator, Indent, SliceDisplayer},
  js_doc::{JsDoc, JsDocTag},
  node::{DocNode, DocNodeKind},
};
...
impl<'a> DocPrinter<'a> {
  pub fn new(doc_nodes: &[DocNode], use_color: bool, private: bool) -> DocPrinter {
    DocPrinter {
      doc_nodes,
      use_color,
      private,
  ...
  fn format_(&self, w: &mut Formatter<'_>, doc_nodes: &[DocNode], indent: i64) -> FmtResult {
    if self.use_color {
      colors::enable_color();
    }
  ...
  fn format_signature(&self, w: &mut Formatter<'_>, node: &DocNode, indent: i64) -> FmtResult {
    match node.kind {
      ...
      DocNodeKind::Interface => self.format_interface_signature(w, node, indent),
      DocNodeKind::TypeAlias => self.format_type_alias_signature(w, node, indent),
      DocNodeKind::Namespace => self.format_namespace_signature(w, node, indent),
      DocNodeKind::Import => Ok(()),
    }
  }

  fn format_jsdoc(&self, w: &mut Formatter<'_>, js_doc: &JsDoc, indent: i64) -> FmtResult {
  ...
  fn format_jsdoc_tag(&self, w: &mut Formatter<'_>, tag: &JsDocTag, indent: i64) -> FmtResult {
    match tag {
  ...
  fn format_interface(&self, w: &mut Formatter<'_>, node: &DocNode) -> FmtResult {
  ...
  fn format_namespace(&self, w: &mut Formatter<'_>, node: &DocNode) -> FmtResult {
  ...
  fn format_enum_signature(&self, w: &mut Formatter<'_>, node: &DocNode, indent: i64) -> FmtResult {
    writeln!(
      w,
      "{}{} {}",
      Indent(indent),
  ...
  fn format_module_doc(&self, _w: &mut Formatter<'_>, _node: &DocNode, _indent: i64) -> FmtResult {
