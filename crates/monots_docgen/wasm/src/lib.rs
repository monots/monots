...
use deno_graph::{
  create_type_graph,
  source::{LoadFuture, Loader, ResolveResponse, Resolver},
  ModuleSpecifier,
};
use monots_docgen::parser::DocParser;
...
impl Resolver for JsResolver {
  fn resolve(&self, specifier: &str, referrer: &ModuleSpecifier) -> ResolveResponse {
    ...
    let value = match self.resolve.call2(&this, &arg0, &arg1) {
      Ok(value) => value,
      Err(_) => return ResolveResponse::Err(anyhow!("JavaScript resolve() function threw.")),
    };
...
pub async fn doc(
  ...
  let entries = DocParser::new(graph, include_all, &source_parser)
    ...
    .map_err(|err| JsValue::from(js_sys::Error::new(&err.to_string())))?;
  JsValue::from_serde(&entries).map_err(|err| JsValue::from(js_sys::Error::new(&err.to_string())))
}
