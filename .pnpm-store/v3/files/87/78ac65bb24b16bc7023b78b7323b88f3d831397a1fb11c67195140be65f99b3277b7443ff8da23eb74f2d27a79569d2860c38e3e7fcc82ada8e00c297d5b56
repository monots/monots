...
const snapshotcache_1 = require("./snapshotcache");
const getsnapshot_1 = require("./getsnapshot");
const utils_1 = require("./utils");
function init(modules) {
    const ts = modules.typescript;
    const config = config_1.defaultConfig;
    const snapshotCache = new snapshotcache_1.SnapshotResolver(ts);
    snapshotCache.extensions = config_1.defaultConfig.snapshotFileExtensions;
    snapshotCache.dir = config_1.defaultConfig.snapshotDir;
    /**
     * Create TS plugin
     */
    function create(info) {
        ...
        proxy.getQuickInfoAtPosition = (fileName, position) => {
            ...
            const snapshotDef = getsnapshot_1.tryGetSnapshotForPosition(ts, sourceFile, position, snapshotCache, config, program);
            if (snapshotDef) {
                return {
                    ...
                    tags: config.useJSTagsForSnapshotHover
                        ...
                        ? utils_1.formatSnapshot(snapshotDef.snapshot.trim(), /*config.extractCSSForJSTag*/ false).map(def => ({
    ...
    function getExternalFiles(project) {
        ...
        let openFiles = [];
        if (typeof project.projectService.openFiles.map === "function") {
            // TS 2.6, openFiles is array of ScriptInfo
            openFiles.push(...project.projectService.openFiles.map(f => f.fileName));
        }
            ...
            openFiles.push(...project.projectService.openFiles.keys());
        }
