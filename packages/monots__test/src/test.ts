...
import { AssertionError, Assertions, checkAssertionMessage } from './test-assertions.js';
  ...
  constructor(test) {
    ...
    this.log = (...inputArgs) => {
      const args = inputArgs.map((value) =>
        typeof value === 'string' ? value : concordance.format(value, concordanceOptions),
      );
    ...
    this.try = async (...attemptArgs) => {
      ...
      } = await test.runAttempt(attemptTitle, (t) => implementation(t, ...args));
...
export class Test {
  constructor(options) {
    ...
    this.runAttempt = async (title, fn) => {
      ...
      const {
        ...
        snapshotBelongsTo,
        nextSnapshotIndex,
        snapshotCount: startingSnapshotCount,
      } = this;
      ...
      const { deferredSnapshotRecordings, error, logs, passed, assertCount, snapshotCount } =
        await attempt.run();
  ...
  countPassedAssertion() {
    ...
    if (this.pendingAttemptCount > 0) {
      this.saveFirstError(
        new Error(
          'Assertion passed, but an attempt is pending. Use the attempt’s assertions instead',
        ),
      );
    }
  ...
  addPendingAssertion(promise) {
    ...
    if (this.pendingAttemptCount > 0) {
      this.saveFirstError(
        new Error(
          'Assertion started, but an attempt is pending. Use the attempt’s assertions instead',
        ),
      );
    }
  ...
  addFailedAssertion(error) {
    ...
    if (this.pendingAttemptCount > 0) {
      this.saveFirstError(
        new Error(
          'Assertion failed, but an attempt is pending. Use the attempt’s assertions instead',
        ),
      );
    }
  ...
  finishAttempt({
    ...
    if (this.finishing) {
      if (commit) {
        this.saveFirstError(
          new Error('`t.try()` result was committed, but the test has already finished'),
        );
      } else {
        this.saveFirstError(
          new Error('`t.try()` result was discarded, but the test has already finished'),
        );
      }
    }
    ...
    if (commit) {
      ...
      if (startingSnapshotCount === this.snapshotCount) {
        ...
        this.saveFirstError(
          new Error(
            'Cannot commit `t.try()` result. Do not run concurrent snapshot assertions when using `t.try()`',
          ),
        );
      }
    }
  ...
  verifyPlan() {
    if (!this.assertError && this.planCount !== null && this.planCount !== this.assertCount) {
      this.saveFirstError(
        new AssertionError({
          assertion: 'plan',
          message: `Planned for ${this.planCount} ${plur('assertion', this.planCount)}, but got ${
            this.assertCount
          }.`,
          operator: '===',
  ...
  verifyAssertions() {
    ...
    if (this.pendingAttemptCount > 0) {
      this.saveFirstError(
        new Error('Test finished, but not all attempts were committed or discarded'),
      );
      return;
    }
  ...
  run() {
    ...
    const returnedObservable =
      result.retval !== null &&
      typeof result.retval === 'object' &&
      typeof result.retval.subscribe === 'function';
