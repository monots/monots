...
function validateExpectations(assertion, expectations, numberArgs) {
  ...
  constructor({
    ...
    this.like = withSkip((actual, selector, message) => {
      ...
      const actualDescriptor =
        result.actual || concordance.describe(comparable, concordanceOptions);
      const expectedDescriptor =
        result.expected || concordance.describe(selector, concordanceOptions);
      fail(
        new AssertionError({
          assertion: 'like',
          message,
        ...
        const intermediate = Promise.resolve(promise).then(
          (value) => {
            ...
            throw new AssertionError({
              ...
              values: [
                formatWithLabel(
                  `${wasReturned ? 'Returned promise' : 'Promise'} resolved with:`,
                  value,
                ),
              ],
            });
      ...
      if (actual) {
        fail(
          new AssertionError({
            ...
            values: [
              formatWithLabel('Function threw synchronously. Use `t.throws()` instead:', actual),
            ],
          }),
        );
        return;
      ...
      const handlePromise = async (promise, wasReturned) => {
        ...
        const intermediate = Promise.resolve(promise).then(noop, (error) => {
          ...
          throw new AssertionError({
            ...
            values: [
              formatWithLabel(
                `${wasReturned ? 'Returned promise' : 'Promise'} rejected with:`,
                error,
              ),
            ],
          });
      ...
      if (result.actual) {
        ...
        fail(
          new AssertionError({
            assertion: 'snapshot',
            message:
              message || 'No snapshot available — new snapshots are not created in CI environments',
          }),
        );
      }
    ...
    this.assert =
      powerAssert === undefined
        ? withSkip((actual, message) => {
