import type { Assertions } from './assertion-types';
import type { Subscribable } from './subscribable-types';
...
export type TitleFn<Args extends unknown[]> = (
  providedTitle: string | undefined,
  ...args: Args
) => string;

/** A reusable test or hook implementation. */
export interface Macro<Args extends unknown[], Context = unknown> {
...
export type Implementation<Args extends unknown[], Context = unknown> =
...
export interface MacroDeclarationOptions<Args extends unknown[], Context = unknown> {
  /** The function that is executed when the macro is used. */
...
export interface MacroFn<Context = unknown> {
  ...
  <Args extends unknown[]>(declaration: MacroDeclarationOptions<Args, Context>): Macro<
    Args,
    Context
