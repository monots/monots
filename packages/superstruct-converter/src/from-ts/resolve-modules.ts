...
export function resolveModules(sourceText: string): SourceFile {
  ...
  const { transformed } = ts.transform(sourceFile, [moduleToPrefix(declarations)]);

  const transformedSourceFile = transformed[0];
  assert(transformedSourceFile, 'The transformed source file does not exist.');
...
function getDeclarationNames(sourceFile: ts.SourceFile) {
  ...
  const extractNamespacedTypesVisitor = (namespace: string) => (node: ts.Node) => {
    if (
...
const moduleToPrefix =
  (declarationNames: Map<string, string[]>): ts.TransformerFactory<ts.SourceFile> =>
  (context) =>
  (sourceFile) => {
      ...
      (node: ts.Node): ts.Node | undefined => {
        if (
          ...
          return f.updateTypeReferenceNode(
            node,
            f.createIdentifier(pascalCase(moduleName) + pascalCase(node.typeName.text)),
            node.typeArguments,
          );
        }
          ...
          return f.updateTypeAliasDeclaration(
            ...
            ts.isTypeLiteralNode(node.type)
              ? f.updateTypeLiteralNode(
                  node.type,
                  ts.visitNodes(node.type.members, prefixInterfacesAndTypes(moduleName)),
                )
              : ts.isTypeReferenceNode(node.type)
              ? f.updateTypeReferenceNode(
                  node.type,
                  node.type.typeName,
                  ts.visitNodes(node.type.typeArguments, prefixInterfacesAndTypes(moduleName)),
                )
              : node.type,
          );
        }
