/*
Extracted from https://github.com/ggascoigne/prettier-plugin-import-sort/blob/master/src/index.js
*/
...
const getAndCheckConfig = (extension: string, fileDirectory: string) => {
  ...
  invariant(resolvedConfig, msg(`No configuration found for file type ${extension}`));
  ...
  invariant(style ?? style === rawStyle, `Style "${rawStyle}" could not be resolved`);
...
const organizeImports = (unsortedCode: string, extension: string) => {
  ...
  const config = getAndCheckConfig(extension, path.resolve(__dirname, '..', '..'));
  const { parser = DEFAULT_PARSER, style = DEFAULT_STYLE, config: rawConfig } = config;
