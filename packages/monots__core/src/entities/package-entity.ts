...
import { OUTPUT_FOLDER, SOURCE_FOLDER_IDENTIFIER, SOURCE_FOLDER_NAME } from '../constants.js';
import {
...
import { Entrypoint, EntrypointField, ExportsField, Package, PackageMonots } from '../schema.js';
...
export class PackageEntity extends BaseEntity<Package> {
  ...
  get fields(): EntrypointField[] {
    return (this.#fields ??= keys(Entrypoint.schema).filter(
  ...
  async createTsConfigs(): Promise<string[]> {
    ...
    const isSourceFolder = (name: string) =>
      [sourceFolderName, `./${sourceFolderName}`].includes(name);

    for (let [folder, content] of entries(tsconfigs)) {
      if (folder === SOURCE_FOLDER_IDENTIFIER) {
