...
import { flowTemplate, tsTemplate, validFields } from './utils';
...
async function getTypeSystem(
  ...
  const content = await fs.readFile(entrypoint.source, 'utf8');
...
async function entrypointHasDefaultExport(entrypoint: Entrypoint, content: string) {
...
export async function writeDevTSFile(entrypoint: Entrypoint, entrypointSourceContent: string) {
  const cjsDistPath = path
    .join(entrypoint.directory, validFields.main(entrypoint))
    .replace(/\.js$/, '.d.ts');

  const output = await (entrypoint.package.project.experimentalFlags
    .typeScriptProxyFileWithImportEqualsRequireAndExportEquals
    ? `import mod = require(${JSON.stringify(
        path.relative(path.dirname(cjsDistPath), entrypoint.source).replace(/\.tsx?$/, ''),
      )});\n\nexport = mod;\n`
...
async function writeTypeSystemFile(
  ...
  const [typeSystem, content] = await typeSystemPromise;

  if (typeSystem === undefined) {
    return;
  }

  const cjsDistPath = path.join(entrypoint.directory, validFields.main(entrypoint));
  ...
  if (typeSystem === 'flow') {
    ...
    await fs.writeFile(
      `${cjsDistPath}.flow`,
      flowTemplate(false, path.relative(path.dirname(cjsDistPath), entrypoint.source)),
    );
  }
...
export default async function dev(projectDir: string) {
  const project = await Project.create(projectDir);
    ...
    project.packages.map((pkg) => {
      return Promise.all(
        pkg.entrypoints.map(async (entrypoint) => {
          const typeSystemPromise = getTypeSystem(entrypoint);
...
`,
          ...
          if (entrypoint.json.browser) {
            const browserField = validFields.browser(entrypoint);

            for (const key of Object.keys(browserField)) {
              promises.push(
                fs.symlink(entrypoint.source, path.join(entrypoint.directory, browserField[key])),
              );
            }
          }
