...
const fields = ['version', 'description', 'main', 'module', 'umd:main', 'browser'];
...
export function getEntrypointName(pkg: Package, entrypointDir: string) {
  return normalizePath(
    nodePath.join(
      pkg.name,
      nodePath.relative(pkg.directory, nodePath.resolve(pkg.directory, entrypointDir)),
    ),
  );
}
...
export const validFieldsFromPkg = {
  main(pkg: Package, entrypointName: string, forceStrategy?: DistFilenameStrategy) {
    const safeName = getDistName(pkg, entrypointName, forceStrategy);
    return `dist/${safeName}.cjs.js`;
  },
  module(pkg: Package, entrypointName: string, forceStrategy?: DistFilenameStrategy) {
    const safeName = getDistName(pkg, entrypointName, forceStrategy);
    return `dist/${safeName}.esm.js`;
  },
  'umd:main'(pkg: Package, entrypointName: string, forceStrategy?: DistFilenameStrategy) {
    const safeName = getDistName(pkg, entrypointName, forceStrategy);
