...
test('all the build types', async () => {
  ...
  let tmpPath = await testdir({
    'package.json': JSON.stringify({
      ...
      browser: {
        './dist/all-the-build-types.cjs.js': './dist/all-the-build-types.browser.cjs.js',
        './dist/all-the-build-types.esm.js': './dist/all-the-build-types.browser.esm.js',
      },
    }),
    'src/index.js': js`
  ...
  expect(
    (await fs.readFile(path.join(distPath, 'all-the-build-types.cjs.js'), 'utf-8')).replace(
      path.relative(tmpPath, path.dirname(require.resolve('@monots/hook'))),
...
test('flow', async () => {
  ...
  expect(
    await fs.readFile(path.join(tmpPath, 'dist', 'flow-dev.cjs.js.flow'), 'utf8'),
  ).toMatchSnapshot();

  expect(
    await fs.readFile(path.join(tmpPath, 'a', 'dist', 'flow-dev-a.cjs.js.flow'), 'utf8'),
  ).toMatchSnapshot();

  expect(
    await fs.readFile(path.join(tmpPath, 'b', 'dist', 'flow-dev-b.cjs.js.flow'), 'utf8'),
  ).toMatchSnapshot();
});
...
test('typescript', async () => {
  ...
  await expect(fs.readFile(path.join(tmpPath, 'dist', 'typescript.cjs.d.ts'), 'utf8')).resolves
    .toMatchInlineSnapshot(`
