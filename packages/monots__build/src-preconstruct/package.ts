...
function getFieldsUsedInEntrypoints(
  descriptors: Array<{ contents: string | undefined; filename: string }>,
) {
  const fields = new Set<keyof typeof validFields>(['main']);

  for (const descriptor of descriptors) {
    if (descriptor.contents !== undefined) {
      const parsed = jsonParse(descriptor.contents, descriptor.filename);

      for (const field of ['module', 'umd:main', 'browser'] as const) {
...
function getPlainEntrypointContent(
  ...
  const obj: Partial<Record<keyof typeof validFields, string | Record<string, string>>> = {};
...
function createEntrypoints(
  ...
  const fields = getFieldsUsedInEntrypoints(descriptors);

  return Promise.all(
    descriptors.map(async ({ filename, contents, hasAccepted, sourceFile }) => {
      if (contents === undefined) {
        if (!hasAccepted) {
          const entrypointName = getEntrypointName(pkg, nodePath.dirname(filename));
          const shouldCreateEntrypointPkgJson = await confirms.createEntrypointPkgJson({
            name: entrypointName,
          });
        ...
        contents = getPlainEntrypointContent(pkg, fields, nodePath.dirname(filename), pkg.indent);
...
export class Package extends Item<{
  ...
  static async create(directory: string, project: Project, isFix: boolean): Promise<Package> {
    const filePath = nodePath.join(directory, 'package.json');

    const contents = await fs.readFile(filePath, 'utf-8');
    const pkg = new Package(filePath, contents, project._jsonDataByPath);
    pkg.project = project;
    const entrypoints = await fastGlob(pkg.configEntrypoints, {
    ...
    if (!entrypoints.length) {
      const oldEntrypoints = await fastGlob(pkg.configEntrypoints, {
        cwd: pkg.directory,
    ...
    pkg.entrypoints = await Promise.all(
      entrypoints.map(async (sourceFile) => {
        ...
        if (
          !normalizePath(sourceFile).includes(normalizePath(nodePath.join(pkg.directory, 'src')))
        ) {
          throw new FatalError(
        ...
        const filename = nodePath.join(directory, 'package.json');
      ...
      if (globErrors.length) {
        const errors = globErrors.map((globError) => {
          if (globError.exists) {
